...
gmd(
  {
    pattern: "kickall",
    aliases: ["removeall", "massremove", "masskick"],
    react: "🚫",
    category: "group",
    description: "Remove multiple mentioned users from the group at once.",
  },
  async (from, Guru, conText) => {
    const {
      reply,
    ...
    if (!mentionedJid || mentionedJid.length === 0) {
      return reply(
        "❌ Mention the users you want to remove!\nExample: .kickall @user1 @user2 @user3",
      );
    }
    ...
    for (const raw of mentionedJid) {
      let targetJid = await convertLidToJid(raw);
      if (!targetJid || targetJid.includes("@lid")) {
        skipped.push(raw.split("@")[0]);
        continue;
      }
      if (!targetJid.includes("@")) targetJid += "@s.whatsapp.net";
      const targetNum = targetJid.split("@")[0];
    ...
    await react(removed.length > 0 ? "✅" : "❌");

    let msg = `🚫 *Kick All Result*\n\n✅ Removed: ${removed.length}\n❌ Skipped: ${skipped.length}`;
    if (removed.length > 0) {
      msg += `\n\n*Removed:*\n${removed.map((j) => `• @${j.split("@")[0]}`).join("\n")}`;
    }
    if (skipped.length > 0) {
      msg += `\n\n*Skipped (admin/creator/failed):*\n${skipped.map((n) => `• @${n}`).join("\n")}`;
    }

    return reply(msg, { mentions: [...removed, ...skipped.map((n) => n + "@s.whatsapp.net")] });
  },
);
...
┃━━━━━━━━━━━━━━━━━━━━━━━━━━━━┃
┃  *What it does:*
┃  • Detects bot commands (30+ prefix styles)
┃  • Catches bare bot keywords (menu, ping…)
