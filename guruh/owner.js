...
gmd(
  ...
  async (from, Guru, conText) => {
    ...
    if (rawTarget.endsWith("@lid")) {
      const cached = getLidMapping(rawTarget);
      if (cached) {
        rawTarget = cached;
      } else {
        try {
    ...
    if (rawTarget.endsWith("@lid")) {
      await react("❌");
      return reply(
        "❌ Could not resolve this user's real number (unknown @lid). Try providing their phone number directly, e.g. `.block 254712345678`.",
      );
    }
...
gmd(
  ...
  async (from, Guru, conText) => {
    ...
    if (rawTarget.endsWith("@lid")) {
      await react("❌");
      return reply(
        "❌ Could not resolve this user's real number (unknown @lid). Try providing their phone number directly, e.g. `.unblock 254712345678`.",
      );
    }
  ...
  async (from, Guru, conText) => {
    ...
    try {
      ...
      const forwardContextInfo = {
        ...
        isForwarded: true,
        forwardedNewsletterMessageInfo: {
          newsletterJid: newsletterJid || "120363408668355773@newsletter",
